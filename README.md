...
> `/uploads` 는 개발 프록시와 컨테이너 nginx 에만 있고 **운영 ALB Ingress 에는 아직 규칙이 없다.** user Pod 의 업로드 볼륨도 `emptyDir` 라 Pod 가 사라지면 함께 사라진다. 업로드 파일은 S3 전환이 남은 과제다.
